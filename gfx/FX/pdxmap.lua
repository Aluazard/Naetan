...
static const float TERRAINTILEFREQ = 182.0f;
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	sample.rgb = GetOverlay( sample.rgb, TerrainColor, 0.8f );
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	if ( terrain_color.a < 1.0f ) //Terrain mapmode
	{
		vOut = sample.rgb;
		vOut = ApplySnow( vOut, Input.prepos, normal, vFoWColor, FoWDiffuse );
	}
	else //Political mapmode
	{
	...
	sample.rgb = ApplySnow( sample.rgb, Input.prepos, normal, vFoWColor, FoWDiffuse );
	#else
	sample.rgb = ApplySnow( sample.rgb, Input.prepos, normal, vFoWColor, FoWDiffuse, 0.8f );
	#endif // PDX_OPENGL

	float2 vBlend = float2( .95f - colorRange ,  colorRange); // Dynamic version
	//float2 vBlend = float2( 0.7f, 0.3f );
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	float3 normal = normalize( tex2D( HeightNormal,Input.uv2 * vMapSize.zw ).rbg - 0.5f );
	float3 sample = tex2D( TerrainDiffuse, Input.uv2 * float2(( vMapSize.x / 32.0f ), ( vMapSize.y / 32.0f ) ) ).rgb;
