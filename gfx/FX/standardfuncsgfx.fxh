...
// Standard functions
//float3 CalculateLighting( float3 vColor, float3 vNormal, float3 vLightDirection, float3 vAmbient, float3 STANDARD_vDiffuseLight, float STANDARD_vIntensity )
//{
...
//	vHalfLambert *= vLightIntensity * sRGBtoRGB( vColor );
//	vHalfLambert = ToneMap ( vHalfLambert );
//	return RGBtosRGB( vHalfLambert );
//}
//
//float3 CalculateLighting( float3 vColor, float3 vNormal )
//{
//	return CalculateLighting( vColor, vNormal, vLightDir, AMBIENT, STANDARD_vDiffuseLight, STANDARD_vIntensity );
//}
...
float3 ApplyDistanceFog( float3 vColor, float3 vPos )
{
	...
	float vFogFactor = 1.15f - vFogDynamic - normalize( vDiff ).y;
